The intent of this hook is to run `pi-patcher reconcile` after pi updates itself, when the user runs the built in `pi update` command.

After pi's built-in `pi update` command finishes updating pi, automatically run `pi-patcher reconcile` so all installed source patches are re-applied to the freshly updated pi install.
