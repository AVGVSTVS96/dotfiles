...
type NoteEntryData =
	| { text: string; prefix: string; createdAt: number }
	| { deleted: true };
...
export default function (pi: ExtensionAPI) {
	...
	function reconstruct(ctx: ExtensionContext): void {
		...
		for (const entry of ctx.sessionManager.getBranch()) {
			...
			if ("deleted" in data && data.deleted) {
				...
				currentNote = { text: data.text, prefix: data.prefix };
			}
		}
	}
	...
	function renderShelf(ctx: ExtensionContext): void {
		...
		ctx.ui.setWidget(
			WIDGET_KEY,
			(_tui, theme) => new NoteShelf(prefix, text, theme),
			...
			if (trimmed === "") {
				...
				if (currentNote !== null) {
					if (ctx.hasUI) {
						...
						ctx.ui.notify(
							`Current note (${currentNote.prefix}): ${currentNote.text}`,
							"info",
						);
					}
					return;
